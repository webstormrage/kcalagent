...
func QueryProduct(input string, token string) (string, error) {
	...
	answer, err := QueryAi(prompt, config, token)
	if err != nil {
		return "", err
	}
	return answer, nil
...
func ParseProduct(data string) (*kcaldb.Product, error) {
	...
	return &kcaldb.Product{
		Name:          strings.TrimSpace(strings.ToLower(product.Name)),
